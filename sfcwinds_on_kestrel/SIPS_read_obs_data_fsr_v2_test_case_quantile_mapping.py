...
hrrr = xr.open_dataset("C:/Users/memes/Documents/SIPS/HRRR/hrrr_US_SW_2025-02-11.nc")   # test case
...
obs['wspd3'] = np.where(obs['height'] == 3, obs['windspeed'], obs['windspeed']*(np.log(3/obs['z0']))/(np.log(obs['height']/obs['z0'])))
obs['wspd10'] = np.where(obs['height'] == 10, obs['windspeed'], obs['windspeed']*(np.log(10/obs['z0']))/(np.log(obs['height']/obs['z0'])))
...
obs_ws_10m_array = np.array(obs['wspd10'])
obs_ws_3m_array = np.array(obs['wspd3'])
...
#%% Test case by station (single)
#for stid in obs["station_id"].unique():
    #station_obs_test2 = obs[obs["station_id"] == stid]
station_obs_test2 = obs[obs["station_id"] == "SVAN5"]       # single station
...
lat2d = hrrr_testcase.latitude.isel(valid_time=0)
lon2d = hrrr_testcase.longitude.isel(valid_time=0)

# Remove the time dimension
lat2d = lat2d.drop_vars("valid_time", errors="ignore")
lon2d = lon2d.drop_vars("valid_time", errors="ignore")

# Assign back as static coordinates
hrrr_testcase = hrrr_testcase.drop_vars(["latitude", "longitude"])
hrrr_testcase = hrrr_testcase.assign_coords(latitude=lat2d, longitude=lon2d)

hrrr_testcase = hrrr_testcase.sortby('valid_time')
valid_times_coord = hrrr_testcase['valid_time']
valid_times_array = valid_times_coord.values
hrrr_testcase['longitude'] = xr.where(hrrr_testcase['longitude'] > 180, hrrr_testcase['longitude'] - 360, hrrr_testcase['longitude'])
hrrr_testcase["wspd10"] = (hrrr_testcase.u10**2 + hrrr_testcase.v10**2)**0.5
...
un10, vn10 = rotate_to_true_north(hrrr_testcase["u10"], hrrr_testcase["v10"], hrrr_testcase["longitude"])
hrrr_testcase = hrrr_testcase.assign(un10=un10, vn10=vn10)


hrrr_loc, idx, grid_lat, grid_lon, fsr_val = find_closest_HRRR_loc(hrrr_testcase, [station_obs_test2.lat.iloc[0], station_obs_test2.lon.iloc[0]])
...
# Add prefixes before merging
df_hrrr_prefix = df_hrrr.add_prefix("hrrr_")
station_obs2_15min_prefix = station_obs2_15min.add_prefix("obs_")

    # Merge on aligned timestamps
merged = pd.merge(
    df_hrrr_prefix.reset_index(),
    station_obs2_15min_prefix.reset_index(),
    left_on="hrrr_timestamp",
    right_on="timestamp",
    how="inner"
)
...
merged["hrrr_wdir10"] = np.degrees(np.arctan2(merged["hrrr_u10"], merged["hrrr_v10"])) +180
merged["hrrr_wdir10_corr"] = np.degrees(np.arctan2(merged["hrrr_un10"], merged["hrrr_vn10"])) +180

merged.rename(columns={"hrrr_heightAboveGround": "hrrr_height"}, inplace=True)
merged.rename(columns={"obs_lat": "obs_latitude"}, inplace=True)
merged.rename(columns={"obs_lon": "obs_longitude"}, inplace=True)
merged.rename(columns={"obs_windspeed": "obs_wspd"}, inplace=True)
merged.rename(columns={"obs_winddirection": "obs_wdir"}, inplace=True)

merged["obs_u10"] = -merged["obs_wspd10"]*np.sin(np.radians(merged["obs_wdir"]))
merged["obs_v10"] = -merged["obs_wspd10"]*np.cos(np.radians(merged["obs_wdir"]))
merged["obs_u3"] = -merged["obs_wspd3"]*np.sin(np.radians(merged["obs_wdir"]))
merged["obs_v3"] = -merged["obs_wspd3"]*np.cos(np.radians(merged["obs_wdir"]))

def wind_uv(ws, wdir):
    wdir_rad = np.radians(wdir)
    u = -ws * np.sin(wdir_rad)
    v = -ws * np.cos(wdir_rad)
    return u, v
...
hrrr_windspeed_10m = merged.hrrr_wspd10[~np.isnan(merged.hrrr_wspd10)]
...
hrrr_winddirection_10m = merged.hrrr_wdir10[~np.isnan(merged.hrrr_wdir10)]
hrrr_winddirection_corr_10m = merged.hrrr_wdir10_corr[~np.isnan(merged.hrrr_wdir10_corr)]
...
hist2 = plt.hist(hrrr_winddirection_corr_10m, bins=np.arange(0, 360, 2), edgecolor='none',weights=np.ones(len(hrrr_winddirection_corr_10m))/len(hrrr_winddirection_corr_10m), density=False,alpha=0.5,label='hrrr 10m')
...
plt.figure()
hist1 = plt.hist(obs_winddirection_10m, bins=np.arange(0, 360, 2), edgecolor='none',weights=np.ones(len(obs_winddirection_10m))/len(obs_winddirection_10m), density=False,alpha=0.5,label= f"{stid}"' 3m')
hist2 = plt.hist(hrrr_winddirection_10m, bins=np.arange(0, 360, 2), edgecolor='none',weights=np.ones(len(hrrr_winddirection_10m))/len(hrrr_winddirection_10m), density=False,alpha=0.5,label='hrrr uncorr 10m')
...
#lon = station_obs_test[0,8]
    ...
    fig, ax = plt.subplots()
    ax.plot(merged.valid_time[~np.isnan(merged.obs_wdir)],merged.obs_wdir[~np.isnan(merged.obs_wdir)],label= f"{stid}"' 3m')
    ax.plot(merged.valid_time[~np.isnan(merged.obs_wdir)],merged.hrrr_wdir10[~np.isnan(merged.obs_wdir)],label='HRRR uncorr 10m')
    ax.plot(merged.valid_time[~np.isnan(merged.obs_wdir)],merged.hrrr_wdir10_corr[~np.isnan(merged.obs_wdir)],label='HRRR corr 10m')
    ax.legend(loc='upper right',fontsize=10)
    ax.set_xlabel('Date')
    ax.set_ylabel('Wind direction (deg)')
    ax.legend(loc='upper right',fontsize=10)
